...
class ClosedRowPolicy : public IRowPolicy, public Implementation {
  ...
  public:
    ...
    void update(bool request_found, ReqBuffer::iterator& req_it) override {
      ...
      auto has_valid_level = [&](int level) {
        return level >= 0 && static_cast<size_t>(level) < req_it->addr_vec.size();
      };
      auto has_valid_flat_bank = [&]() {
        return has_valid_level(m_rank_level) &&
               has_valid_level(m_bankgroup_level) &&
               has_valid_level(m_bank_level) &&
               req_it->addr_vec[m_rank_level] >= 0 &&
               req_it->addr_vec[m_bankgroup_level] >= 0 &&
               req_it->addr_vec[m_bank_level] >= 0;
      };
      auto flat_bank_id = [&]() {
        return req_it->addr_vec[m_bank_level] +
      ...
      {  
        ...
        } else if (has_valid_flat_bank()) {  // single bank closes  (PRE, VRR, RDA, WRA)
          int bank_slot = flat_bank_id();
          if (bank_slot >= 0 && bank_slot < static_cast<int>(m_col_accesses.size())) {
            m_col_accesses[bank_slot] = 0;
          }
        }
      } else if (m_dram->m_command_meta(req_it->command).is_accessing)  // RD or WR
      {
        if (!has_valid_flat_bank()) {
          return;
        }
        int bank_slot = flat_bank_id();
        if (bank_slot < 0 || bank_slot >= static_cast<int>(m_col_accesses.size())) {
        ...
        m_col_accesses[bank_slot]++;

        if (m_col_accesses[bank_slot] >= m_cap) {
