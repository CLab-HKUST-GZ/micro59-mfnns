...
struct Request { 
  ...
  int source_id = 0;  // An identifier for where the request is coming from (e.g., which core)
  ...
  int n_command = 0;
  std::string command_history = "";   // The history of commands that have been issued to this request
  ...
  Clk_t issue_time = -1;   // Clock cycle when the request is issued to the memory controller
  ...
  TravReq trav;
  PDisReq anns;
  bool is_anns_graph = false;
  ANNSCpuRequest cpuanns;
  struct MemoryLocation {
    uint32_t channelId;
    uint32_t rankId;
    uint32_t bankgroupId;
    uint32_t bankId;
  };
  MemoryLocation ndpLoc;
  uint64_t id;
  ...
  Request(Addr_t addr, int type, int source_id, std::function<void(Request&)> callback, PDisReq anns);
  Request(Addr_t addr, int type, int source_id, std::function<void(Request&)> callback, PDisReq anns,bool nocallback);
  Request(Addr_t addr, int type, int source_id, std::function<void(Request&)> callback, ANNSCpuRequest cpuanns);
  Request(Addr_t addr, int type, int source_id, std::function<void(Request&)> callback, TravReq trav);
  Request(Addr_t addr, int type, int source_id, std::function<void(Request&)> callback, uint64_t id);
};
...
struct ReqBuffer {
  ...
  bool full() const { return buffer.size() >= max_size; }
