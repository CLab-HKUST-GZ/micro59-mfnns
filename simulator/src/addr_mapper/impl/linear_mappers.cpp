...
class LinearMapperBase : public IAddrMapper {
  ...
  protected:
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) {
      ...
      printf("DDR Linear Mapper: m_num_levels %d\n", m_num_levels);
      uint32_t bit_offset = calc_log2(m_dram->m_internal_prefetch_size);
      std::string level_map[] = {"channel", "rank", "bankgroup", "bank", "row", "col"};
      for (uint32_t id = m_addr_bits.size() - 1; id >= 1; id--) {
        printf("[AddrMapper] id %d Level %s bits range: %d-%d\n",id, level_map[id].c_str(), bit_offset+m_addr_bits[id]-1, bit_offset);
        bit_offset += m_addr_bits[id];
      }
      fflush(stdout);
    }
...
class RoBaRaChCo final : public LinearMapperBase, public Implementation {
  ...
  public:
    ...
    void apply(Request& req) override {
      ...
      for (int i = 1; i <= m_row_bits_idx; i++) {
        req.addr_vec[i] = slice_lower_bits(addr, m_addr_bits[i]); // rank -> row
      }
    }
};

class NDPPerBank final : public LinearMapperBase, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IAddrMapper, NDPPerBank, "NDPPerBank", "Applies a NDPPerBank mapping to the address.");
  ...
  public:
    ...
    void apply(Request& req) override {
      ...
      Addr_t addr = req.addr >> m_tx_offset;
      req.addr_vec[0] = slice_lower_bits(addr, m_addr_bits[0]); // channel
      req.addr_vec[1] = slice_lower_bits(addr, m_addr_bits[1]); // rank
      req.addr_vec[0] = req.ndpLoc.channelId;
      req.addr_vec[1] = req.ndpLoc.rankId;
      for (int i = 2; i <= m_addr_bits.size() - 1; i++) {
        req.addr_vec[i] = slice_lower_bits(addr, m_addr_bits[i]); // bankgroup -> col
      }
      // printf("NDPPerBank: addr %lu, mid %d, channel %d, rank %d, bankgroup %d\n", req.addr, req.anns.embUnitId, req.addr_vec[0], req.addr_vec[1], req.addr_vec[2]);
    }
};
