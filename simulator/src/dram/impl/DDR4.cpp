...
class DDR4 : public IDRAM, public Implementation {
  ...
  public:
    ...
    void issue_command(int command, const AddrVec_t& addr_vec) override {
      //printf("DDR4 issue_command id:%s \n",get_id().c_str());
  ...
  private:
    ...
    void set_powers() {
      ...
      // printf("DRAM Power Model: %s\n", m_drampower_enable ? "Enabled" : "Disabled");
      // fflush(stdout);
    ...
    void finalize() override {
      ...
      // printf("Total Background Energy: %f\n", s_total_background_energy);
      // fflush(stdout);
    }
    ...
    void process_rank_energy(PowerStats& rank_stats, Node* rank_node) {
      ...
      rank_stats.total_cmd_energy = act_cmd_energy 
                                    ...
                                    + wr_cmd_energy 
                                    + ref_cmd_energy;
      // printf("Rank %d: act_cmd_energy: %f, pre_cmd_energy: %f, rd_cmd_energy: %f, wr_cmd_energy: %f, ref_cmd_energy: %f\n", 
      //         rank_stats.rank_id, act_cmd_energy, pre_cmd_energy, rd_cmd_energy, wr_cmd_energy, ref_cmd_energy);
      // fflush(stdout);
