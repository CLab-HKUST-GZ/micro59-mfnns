...
import MFNNS.Testing.GoldenModels.FPMA_Square_FP16_Golden
...
object Test_FPMA_Square_FP16 {
    ...
    Config.sim
      ...
      .doSim("Test_FPMA_Square_FP16") { dut =>
        for {
          ...
          val actualBits = dut.io.FP16_Out.toInt
          val expectedBits = FPMA_Square_FP16_Golden.calculate(inputBits)
  ...
  private def printHeader(): Unit = {
    println()
    println("=" * 72)
    println("[Test] FPMA_Square_FP16 Normal-Square Bit-Exact Test")
    println("- Input domain : all normal FP16 encodings")
    println("- Operation    : FP16_In = x; FP16_Out = SquareFPMA(x)")
    ...
    println("- Reference    : FPMA_Square_FP16_Golden")
    println("=" * 72)
  }
  ...
  private def printRepresentativeGroupHeader(sign: Int): Unit = {
    val groupName = if (sign == 0) "positive normal inputs: square(+x)" else "negative normal inputs: square(-x)"
    println()
  ...
  private def printComparison(
    ...
    val compensation = FPMA_Square_FP16_Golden.evaluate(inputBits).compensation
  ...
  private def printFinalSummary(stats: TU.TestStats): Unit = {
    ...
    println("-" * FinalSummaryWidth)
    println("  Operation          : FP16_In = x; verify FP16_Out for square(x)")
    ...
    println("  Golden reference   : FPMA_Square_FP16_Golden")
    println("  Pass criterion     : DUT FP16_Out[15:0] equals Golden FP16 bits exactly")
    println("-" * FinalSummaryWidth)
